...
public class NastupDtoToNastup implements  Converter<NastupDTO, Nastup>{
	...
	@Autowired
	private NastupService nastService;
	@Autowired
	
	private IzvodjacService izvService;
	...
	public Nastup convert(NastupDTO source) {
		...
		if(source.getFestivalId() != null) {
			festival = festService.findOne(source.getFestivalId()).get();
		}

		Izvodjac izvodjac = null;
		if(source.getIzvodjacId() != null) {
			izvodjac = izvService.findOne(source.getIzvodjacId()).get();
		}
		
		if(festival!=null ||  izvodjac !=null) {
//			//      JEDNA OD OVE 3 VARIJANTE CE RADITI  		
					Long id = source.getId();
					Nastup nastup = id == null ? new Nastup() : nastService.findOne(id).get();
